...
import { ProductSalesDialog, CustomerDialog } from '../subview/dialog';
...
const ReturnAdd = () => {
  ...
  async function getData(id) {
    await service.getDataById(`return`, id).then((response) => {
      let _ = response.data;
      setCustomer([{ 'CustomerId': _.CustomerId, 'Name': _.customer.Name}]);
      setProducts(_.returndetails);
  ...
  async function create(a, b, c) {
    await service.createData(`return`, {
      ReturnId: '',
      UserId: user.UserId,
      ...
      navigate(`/return/${response.data.ReturnId}`);
      enqueueSnackbar('Return Added', {variant: 'success'});
    })
  }

  async function update(a, b, c) {
    await service.updateData(`return`, c, {
      ReturnId: id,
      CustomerId: customer[0].CustomerId,
      PaymentMethod: payment,
      DebtDue: a,
      ...
      navigate(`/return/${response.data.ReturnId}`);
      enqueueSnackbar('Return Updated', {variant: 'info'});
    })
  }

  const handleBack = (e) => {
    e.preventDefault();
    navigate(`/return/${id}`, {replace: true});
  }
  ...
  function check() {
    for (let i in products) {
      if (products[i].SalesPrice === 0) {
        enqueueSnackbar(`Please input ${products[i].Name} sales price`, {variant: 'error'});
        return false
      } 
    }
    return true
      ...
      <Slide direction='left' in={!slideContent} mountOnEnter unmountOnExit>
        <div className="container shadow-lg mt-4 mb-4 p-4 abbba">
          ...
          <div className='foo-btn'>
            ...
            <Button onClick={createReturn} type='submit' size='sm' className='btn-fill ms-4'>CONFIRM</Button>
          </div>
        </div>
      </Slide>
    </div>
